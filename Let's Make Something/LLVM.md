...
現如今LLVM已有以下應用: LLVM專案(The LLVM Project)、LLVM/Clang(LLVM-Clang C/C++ language Frontend)、Swift等一系列程式語言編譯器、編譯式程式語言編譯器、直譯式語言解釋器、編譯式語言的可直譯化解釋器(Root-Project Cling、LFortran等)、特定架構晶片執行代碼編譯器/轉譯器架構(Intel LLVM DPCPP-sycl(oneAPI DPC++/C++)、AMD Optimized C/C++(AOCC)、NVIDIA CUDA: NVC/C++ NVFortran)等全自定義化語言編譯架構。
...
我們解釋以下開發套件的必要原因:
 ...
 - CMake: 由於LLVM使用CMake(CrossMake)作為大型組織化編譯呼叫的建置工具，因此我們需要安裝CMake，並且VS2022會為該選項配置生成器`Ninja`。該CMake為適用於MSVC的特規版(`3.26.4-msvc-4`)，只有CLI介面(沒有視窗介面)。
